...
implements FileHeader {
    ...
    private long decodeMsDosFormatDateTime(long datetime) {
        int year = CentralDirectoryFileHeader.getChronoValue((datetime >> 25 & 127L) + 1980L, ChronoField.YEAR);
        int month = CentralDirectoryFileHeader.getChronoValue(datetime >> 21 & 15L, ChronoField.MONTH_OF_YEAR);
        int day = CentralDirectoryFileHeader.getChronoValue(datetime >> 16 & 31L, ChronoField.DAY_OF_MONTH);
        int hour = CentralDirectoryFileHeader.getChronoValue(datetime >> 11 & 31L, ChronoField.HOUR_OF_DAY);
        int minute = CentralDirectoryFileHeader.getChronoValue(datetime >> 5 & 63L, ChronoField.MINUTE_OF_HOUR);
        int second = CentralDirectoryFileHeader.getChronoValue(datetime << 1 & 62L, ChronoField.SECOND_OF_MINUTE);
